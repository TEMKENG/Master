...
export class MenuEntry {
    ...
    constructor(entryType?: string, entryName?: string, entryFunction?, select?: boolean) {
        this.select = select;
...
export class menuApi {
    createMenu(menuID): Menu {
        return new Menu(menuID);
    };

    createItem(itemname, entryType, handler, select?: boolean, ok?: boolean): MenuEntry {
    ...
    createSeparator(handler): MenuEntry {
        return new MenuEntry('hr', '', handler);
    }


    createRadioOption(type: string, colors: { [p: string]: string }, color?: string): MenuEntry[] {
