...
interface Shape {
    readonly id: number;
    draw(ctx: CanvasRenderingContext2D, select?: boolean, strokeColour?: number);
...
class Triangle extends AbstractShape implements Shape {
    ...
    move(x: number, y: number) {
        const b1 = this.checkSign(x, y, this.p1, this.p2) < 0.0;
        const b2 = this.checkSign(x, y, this.p2, this.p3) < 0.0;
        const b3 = this.checkSign(x, y, this.p3, this.p1) < 0.0;
