...
import {AddShapeEvent, ChooseShapeAtEvent, RemoveShapeWithIdEvent, ShapeEvent} from "./Events.js";
...
let allCanvas: CanvasObject[] = []; // contains a structure with the Canvas Id and a list of Event
let canvasList = [];
let clientIds = [];
let clientId = "";
let clients = [];
...
function createEvent(e: { [p: string]: any }): ShapeEvent {
...
function reduceEventList(eventE: { [p: string]: any }, events: { [p: string]: any }[]): { [p: string]: any }[] {
    let len = events.length;
    let event = createEvent(eventE);
    let shapeEvents: ShapeEvent [] = [];
...
class CanvasObject {
    constructor(public canvasID: number, public clientList = [],
    ) {
    }

    addClient(borderColor?: string) {
